...
-- Toggle hover_icon_on_border depending on whether the focused window is floating.
-- Enabled for floating windows, disabled for tiled ones.
hl.on("window.active", function(w)
  if w ~= nil and w.floating == true then
    hl.config({ general = { hover_icon_on_border = true } })
  else
    hl.config({ general = { hover_icon_on_border = false } })
  end
end)
...
hl.window_rule({ match = { title = "^(aether Settings)$" }, float = true })
...
hl.layer_rule({ match = { namespace = "quickshell:notificationPopup" }, animation = "spring" })
