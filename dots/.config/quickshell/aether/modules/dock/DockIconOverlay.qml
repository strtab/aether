...
// Taken from iNiR - https://github.com/snowarch/iNiR.git
// Responsibilities:
...
Item {
  ...
  property bool animationsEnabled: Appearance.animationsEnabled ?? true
  ...
  property real dotWidthFocused: Config.options?.dock?.dots?.widthFocused ?? 6
  property real dotWidthInactive: Config.options?.dock?.dots?.widthInactive ?? 6
  property real dotHeight: Config.options?.dock?.dots?.height ?? 6
  property int maxDots: Config.options?.dock?.dots?.maxCount ?? 5
  ...
  Row {
    ...
    property bool smartIndicator: Config.options?.dock?.smartIndicator ?? true
    property bool showAllDots: Config.options?.dock?.showAllWindowDots ?? true
    ...
    Repeater {
      ...
      delegate: Rectangle {
        ...
        property bool isFocused: {
          if (!root.appIsActive)
            return false;
          if (!indicatorRow.smartIndicator)
            return true;
        ...
        implicitWidth: Appearance.angelEverywhere ? (isFocused ? 14 : 6) : (isFocused ? root.dotWidthFocused : root.dotWidthInactive)
        implicitHeight: Appearance.angelEverywhere ? 2 : root.dotHeight
        color: isFocused ? (Appearance.colors.colPrimary) : ColorUtils.transparentize(Appearance.colors.colOnLayer0, 0.5)
    ...
    Rectangle {
      ...
      width: Appearance.angelEverywhere ? 6 : root.dotWidthInactive
      height: Appearance.angelEverywhere ? 2 : root.dotHeight
