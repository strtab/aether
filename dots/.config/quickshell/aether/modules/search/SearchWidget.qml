...
Item { // Wrapper
  ...
  property string emptyPlaceholderText: "No results found."
  ...
  Timer {
    id: showResultsTimer
    interval: 10
    onTriggered: root._resultsReady = true
  }
  ...
  Connections {
    ...
    function onQueryChanged() {
      ...
      if (LauncherSearch.query === "" && LauncherSearch.mode === "") {
        showResultsTimer.stop();
        root._resultsReady = false;
      } else {
        showResultsTimer.restart();
      }
    }
  }
  ...
  Rectangle { // Background
    ...
    Behavior on implicitHeight {
      ...
      enabled: GlobalStates.searchOpen && root.showResults && (Config.options?.search?.contentAnimation ?? true)
      animation: Appearance.animation.searchExpand.numberAnimation.createObject(this)
    }
    ...
    ColumnLayout {
      ...
      ListView { // App results
        ...
        onCurrentIndexChanged: LauncherSearch.selectedIndex = currentIndex
