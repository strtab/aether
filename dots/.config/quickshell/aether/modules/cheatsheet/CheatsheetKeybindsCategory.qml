...
Column {
  ...
  function containsNonFirstRepetitive(bind) {
    const key = bind.key;
    if (key.includes("mouse") || key.includes("page"))
      return false;
  ...
  function transformDescription(bind, categoryName) {
    ...
    if (!containsFirstRepetitive(bind))
      return decategorized;
      ...
      Row {
        ...
        Repeater {
          ...
          delegate: KeyboardKey {
            required property var modelData
            key: root.transformKey(modelData) + " " + root.transformKey(bindLine.keyData.key)
