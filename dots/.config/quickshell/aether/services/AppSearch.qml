...
Singleton {
  ...
  property var blacklisted: {
    const list = Config.options?.search?.blacklist ?? [];
  ...
  property var regexSubstitutions: [
    ...
    {
      "regex": /^(discord|slack|teams|zoom).*$/i,
      "replace": "$1"
    },
    ...
    {
      "regex": /^soffice\..*/,
      "replace": "libreoffice-startcenter"
    },
    {
      "regex": /^libreoffice-.*writer.*/,
      "replace": "libreoffice-writer"
    },
    {
      "regex": /^libreoffice-.*calc.*/,
      "replace": "libreoffice-calc"
    },

    // Flatpak reverse domain
    {
      "regex": /^org\.mozilla\.firefox$/,
      "replace": "firefox"
    },
    {
    ...
    {
      "regex": /^com\.microsoft\.Edge$/,
      "replace": "microsoft-edge"
    },
    {
      "regex": /^io\.github\.AquariusPower\..+/,
      "replace": "application-x-executable"
    },
    ...
    {
      "regex": /.*authentication.*agent.*/i,
      "replace": "dialog-password"
    },

    // Portal/XDG dialogs
    {
      "regex": /xdg-desktop-portal.*/,
      "replace": "preferences-desktop"
    },
    {
      "regex": /org\.freedesktop\.impl\.portal.*/,
      "replace": "preferences-desktop"
    },
  ...
  function _rebuildCache(): void {
    const entries = Array.from(DesktopEntries.applications.values).filter(a => !root.blacklisted[a.id] && !root.blacklisted[a.name]).sort((a, b) => a.name.localeCompare(b.name));
    _cachedList = entries;
    _cachedPreppedNames = entries.map(a => ({
          name: Fuzzy.prepare(`${a.name} `),
          entry: a
        }));
    _cachedPreppedIcons = entries.map(a => ({
  ...
  function fuzzyQuery(search: string): var {
    ...
    const exactPrefixMatches = _cachedList.filter(obj => obj.name?.toLowerCase().startsWith(searchLower));
    ...
    if (root.sloppySearch) {
      ...
      const results = _cachedList.map(obj => {
        ...
        if (nameLower.includes(" " + searchLower) || nameLower.includes("-" + searchLower)) {
          score += 0.15;
        } else
        // Boost for contains
  ...
  function lookupDesktopEntry(appId) {
    ...
    const direct = _startupClassMap[lowered] ?? _execBasenameMap[lowered] ?? _desktopIdStemMap[lowered] ?? _execBasenameMap[kebab] ?? _desktopIdStemMap[kebab];
    if (direct)
      return direct;
    ...
    if (segments.length > 1) {
      ...
      for (const seg of segments) {
        ...
        const found = _execBasenameMap[seg] ?? _desktopIdStemMap[seg] ?? _execBasenameMap[segClean] ?? _desktopIdStemMap[segClean] ?? _startupClassMap[seg] ?? _startupClassMap[segClean];
        if (found)
          return found;
      }
    }
    ...
    if (tokens.length > 0) {
      ...
      for (const [key, mapEntry] of Object.entries(_desktopIdStemMap)) {
        ...
        for (const t of tokens) {
          if (keyTokens.some(kt => kt === t || kt.includes(t) || t.includes(kt)))
            overlap++;
        }
  ...
  function resolveIcon(iconNameOrPath, fallback): string {
    ...
    if (iconNameOrPath.startsWith("/") || iconNameOrPath.startsWith("file://")) {
      ...
      if (path.includes("/Descargas/") || path.includes("/Downloads/") || path.includes("/tmp/") || path.includes("/resources/")) {
